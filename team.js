...
const listItemsID = ["Chairman", "Organising", "Joint","Finance","Publicity","PublicityCo","TechnicalProgram","Publication","Registration","Internet","Sponsorship","Exhibition","International"];
...
const listItemTemplate = (text, translateValue, id = "__blank") => {
  return `
    <li class="dropdown-list-item">
      <a href="#${id}" class="dropdown-button list-button" data-translate-value="${translateValue}%">
...
const renderListItems = () => {
  dropdownList.innerHTML += listItems
    .map((item, index) => {
      return listItemTemplate(item, 100 * index, listItemsID[index]);
    })
    .join("");
};
...
const setDropdownProps = (deg, ht, opacity) => {
  root.style.setProperty("--dropdown-rotate-arrow", deg !== 0 ? deg + "deg" : 0);
  root.style.setProperty("--dropdown-dropdown-height", ht !== 0 ? ht + "rem" : 0);
  root.style.setProperty("--dropdown-list-opacity", opacity);
};
...
mainButton.addEventListener("click", () => {
  ...
  const currDropdownHeight =
    root.style.getPropertyValue("--dropdown-dropdown-height") || "0";
...
dropdownList.addEventListener("mouseover", (e) => {
  ...
  root.style.setProperty("--dropdown-translate-value", translateValue);
});
...
dropdownList.addEventListener("mousemove", (e) => {
  const iconSize = root.style.getPropertyValue("--dropdown-floating-icon-size") || 0;
